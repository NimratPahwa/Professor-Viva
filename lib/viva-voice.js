...
function coreIdentity() {
  return [
    ...
    '',
    'Voice register (who is reading):',
    '- The reader is an Indian developer or aspiring founder, roughly 25 to 45.',
    '  Every joke must be fully self-contained. If understanding a line needs',
    '  startup-culture insider knowledge, rewrite it so anyone gets it.',
    '- Speak DIRECTLY to the founder in second person ("you", "your idea"). This',
    '  is cocky, affectionate trash-talk, a sharp friend teasing them: aimed at',
    '  their DECISIONS and ASSUMPTIONS about this idea, never their intelligence,',
    '  background, or worth. Punch at what they assumed versus what the evidence',
    '  actually found.',
    '- Sarcasm targets the SITUATION, never the person. Cultural references may',
    '  come only from universal Indian founder experiences, never accents or',
    '  stereotypes.',
    '- Banned punctuation and words in everything you write: no em dashes, no',
    '  double hyphens, and none of this jargon: TAM, GTM, ICP, MVP,',
    '  "product-market fit," "demo day." Use plain words and let sentences flow',
    '  with commas, colons, and full stops instead.'
  ].join('\n');
}
...
function taskInstructions(verdict) {
  ...
  return [
    `The verdict is ${verdict}. Deliver it as ONE roast spoken DIRECTLY to the`,
    'founder in second person, 60 to 90 words: the extended screen-bubble cut,',
    'which is a tight 30 to 45 word card roast plus exactly one more beat of',
    'specificity (a detail from the evidence or a second jab at the assumption).',
    'Cocky, affectionate trash-talk at their DECISIONS and ASSUMPTIONS, never the',
    'person. No padding, no softening, no markdown headings, no bullet lists, no',
    'score numbers.',
    doorByVerdict[verdict],
    'Build it from THIS idea\'s own evidence: its real competitor count, its actual',
    'demand findings, and its strongest and weakest scored dimensions. Punch at',
    'what the founder ASSUMED versus what the evidence FOUND. Two different ideas',
    'must never receive the same roast. Every claim you make must trace to a fact',
    'in the evidence block. Do not restate the raw scores; translate what the',
    'evidence MEANS. Do not append a disclaimer or footer yourself; that is added',
    'afterward.'
  ].join('\n');
}
...
async function renderVerdictVoice({ verdict, scores, evidence, idea, dial }) {
  ...
  return {
    ...
    model: VOICE_MODEL,
    usage: usageLine('voice:verdict', VOICE_MODEL, message)
  };
}
...
async function renderNextSteps({ verdict, scores, evidence, idea, dial }) {
  ...
  return {
    ...
    voice_prompt_version: VOICE_PROMPT_VERSION,
    model: VOICE_MODEL,
    usage: usageLine('voice:next_steps', VOICE_MODEL, message)
  };
}

// ── The Six Answers: Answers 5 & 6 (The Professor's Stage §3 Screen 5 / §4.2). ──
// The deep-run report ALWAYS produces two extra ACTION fields the free tier does
// not: Answer 5 (acquisition — "your first ten customers") grounded in the real
// demand channels found on the evidence rows, and Answer 6 (first_revenue —
// "your first dollar") grounded in comparable pricing evidence. Same two-pass
// rule: these narrate actions over an already-decided verdict; they never touch a
// score. Schema-enforced non-empty so a run can never silently omit an answer.
const SixAnswersSchema = z.object({
  acquisition: z.string().min(1),
  first_revenue: z.string().min(1)
});

// Distinct, sourced demand channels found in the evidence (from migration 0009's
// evidence.channel). Answer 5 must ground its plan in THESE, not invented ones.
function demandChannels(evidence) {
  const seen = new Map();
  for (const row of evidence) {
    if (row.dimension === 'demand' && row.channel && row.channel.url) {
      if (!seen.has(row.channel.url)) seen.set(row.channel.url, row.channel);
    }
  }
  return [...seen.values()];
}

// The monetization evidence rows — comparable pricing signals Answer 6 grounds in.
function pricingEvidence(evidence) {
  return evidence.filter((r) => r.dimension === 'monetization');
}

// A grounding block naming the ONLY channels Answer 5 may cite and the ONLY
// pricing facts Answer 6 may cite — so both answers trace to real evidence
// (guardrail 1, screened again at delivery). Empty sets are stated honestly so
// the model tells the truth ("no channel surfaced") rather than inventing one.
function sixAnswersGroundingBlock(evidence) {
  const channels = demandChannels(evidence);
  const pricing = pricingEvidence(evidence);
  const lines = ['GROUNDING FOR ANSWERS 5 & 6 (cite only what appears here):', ''];

  lines.push('Demand channels where real demand was observed (Answer 5 — acquisition):');
  if (channels.length === 0) {
    lines.push('  (no specific channel surfaced — say so honestly; do not invent a community)');
  } else {
    for (const c of channels) lines.push(`  - ${c.name} [${c.type}] (${c.url})`);
  }
  lines.push('');

  lines.push('Comparable pricing evidence (Answer 6 — first revenue):');
  if (pricing.length === 0) {
    lines.push('  (no pricing comparable surfaced — say so honestly; do not invent a number)');
  } else {
    for (const r of pricing) lines.push(`  - [${r.signal}] ${r.claim} (source: ${r.source_url})`);
  }
  return lines.join('\n');
}

function sixAnswersTaskInstructions(verdict) {
  return [
    `The ${verdict} verdict is decided. Produce the report's two ACTION answers.`,
    'ANSWER 5 (acquisition) — "Your first ten customers": a single concrete',
    '  acquisition action the founder can take THIS week to reach their first ten',
    '  customers, grounded in the demand channels listed below. Name the specific',
    '  channel(s). If no channel surfaced, say so plainly and give the honest',
    '  next move (find where they gather first) — never invent a community.',
    'ANSWER 6 (first_revenue) — "Your first dollar": a single concrete path to the',
    '  first paid dollar, grounded in the comparable pricing evidence below',
    '  (a price point to test, a package to sell). If no pricing comparable',
    '  surfaced, say so and give the honest first pricing experiment — never',
    '  invent a number.',
    'Each answer is one tight, doable paragraph in Viva\'s voice at the dial above.',
    'Useful first, never a joke at the founder\'s expense. No headings, no lists,',
    'no score numbers, no invented facts.'
  ].join('\n');
}

function buildSixAnswersPrompt(dial, verdict, scores, evidence) {
  return [
    coreIdentity(),
    contextDial(dial),
    sixAnswersTaskInstructions(verdict),
    evidenceBlock(scores, evidence),
    sixAnswersGroundingBlock(evidence),
    'OUTPUT CONTRACT: Return { "acquisition": string, "first_revenue": string }. ' +
      'Both non-empty plain prose. No headings, no numbering, no score numbers, ' +
      'no closing disclaimer.'
  ].join('\n\n---\n\n');
}

// Validates a Claude response into the schema-enforced six-answers action fields.
// Pure given a message-like object ({ parsed_output }), so it is hermetically
// testable without an LLM call. Throws if either field is missing/empty — a run
// may never omit an answer.
function parseSixAnswers(message) {
  const parsed = message.parsed_output || {};
  const acquisition = String(parsed.acquisition || '').trim();
  const first_revenue = String(parsed.first_revenue || '').trim();
  const result = SixAnswersSchema.parse({ acquisition, first_revenue });
  return result;
}

// Renders Answers 5 & 6 for the already-decided deep verdict in ONE structured
// call. `dial` mirrors the verdict voice's dial (dial-0 for sensitive input).
async function renderReportActions({ verdict, scores, evidence, idea, dial }) {
  const resolvedDial = dial === undefined ? SARCASM_DIAL[verdict] : dial;
  if (resolvedDial === undefined) {
    throw new Error(`renderReportActions: unknown verdict "${verdict}"`);
  }

  const system = buildSixAnswersPrompt(resolvedDial, verdict, scores, evidence);
  ...
  const message = await client.messages
    .stream({
      ...
      max_tokens: 1500,
      system,
      output_config: { format: zodOutputFormat(SixAnswersSchema, 'six_answers') },
      messages: [{
        role: 'user',
        content:
          `The idea under review — problem: "${idea.problem}"; audience: ` +
          `"${idea.audience}". Give Answer 5 (first ten customers) and Answer 6 ` +
          `(first dollar) for the ${verdict} verdict now.`
      }]
    })
    .finalMessage();

  const six_answers = parseSixAnswers(message);
  ...
  return {
    ...
    voice_prompt_version: VOICE_PROMPT_VERSION,
    model: VOICE_MODEL,
    usage: usageLine('voice:six_answers', VOICE_MODEL, message)
  };
}
...
async function renderQuickVerdict({ verdict, scores, evidence, idea, dial }) {
  ...
  return {
    ...
    voice_prompt_version: VOICE_PROMPT_VERSION,
    model: QUICK_VOICE_MODEL,
    usage: usageLine('quick_voice', QUICK_VOICE_MODEL, message)
  };
}
