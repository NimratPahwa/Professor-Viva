...
async function runDeliveryStage(idea) {
  ...
  const render = async (dial) =>
    (await renderVerdictVoice({ verdict: verdictRow.verdict, scores, evidence, idea, dial })).voice;
  ...
  return { verdictRow, guarded, updated, nextSteps: nextStepsResult.next_steps };
}
