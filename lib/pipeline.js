...
const { renderVerdictVoice, renderNextSteps, renderReportActions, SARCASM_DIAL, VOICE_PROMPT_VERSION } = require('./viva-voice');
...
async function runDeliveryStage(idea) {
  ...
  // Screen 5 Answers 5 & 6 (The Professor's Stage §4.2): the deep run ALWAYS
  // produces the acquisition + first-revenue actions, grounded in demand channels
  // / pricing evidence. Same dial + same G1/G2 screening as the next steps; a
  // filtered set regenerates once at dial 0.
  let sixAnswersResult = await renderReportActions({
    verdict: verdictRow.verdict, scores, evidence, idea, dial: guarded.dialUsed
  });
  usage.push(sixAnswersResult.usage);
  const sixText = (sa) => `${sa.acquisition}\n${sa.first_revenue}`;
  let saScreen = screenReply(sixText(sixAnswersResult.six_answers), evidence);
  if (!saScreen.ok) {
    sixAnswersResult = await renderReportActions({
      verdict: verdictRow.verdict, scores, evidence, idea, dial: 0
    });
    usage.push(sixAnswersResult.usage);
    saScreen = screenReply(sixText(sixAnswersResult.six_answers), evidence);
    if (!saScreen.ok) {
      const e = new Error('Six-answer actions failed guardrail screening after dial-0 regeneration.');
      e.code = 'GUARDRAIL_RESIDUAL';
      e.violations = saScreen.violations;
      throw e;
    }
  }
  ...
  await updateVerdictVoice(verdictRow.id, {
    ...
    sixAnswers: sixAnswersResult.six_answers
  });
  ...
  return { verdictRow, guarded, updated, nextSteps: nextStepsResult.next_steps, sixAnswers: sixAnswersResult.six_answers, usage };
}
