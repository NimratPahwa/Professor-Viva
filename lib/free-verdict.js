...
async function renderQuickVerdictGuarded({ verdict, scores, evidence, idea }) {
  ...
  // Meter every voice call, including a dial-0 regeneration.
  const usage = [];
  ...
  return { result, dialUsed, sensitiveInput, regenerated, usage };
}
...
async function produceFreeVerdict(idea, { onDimension } = {}) {
  // onDimension (optional) streams each dimension's finding as it settles, for
  // the live wait screen (Screen 3). It does not change the produced payload.
  const { dimensionResults, evidence, usage: evidenceUsage } = await gatherQuickEvidence(idea, { onDimension });
  ...
  return {
    ...
    // Measured API cost of this free verdict (quick evidence pass + voice call).
    cost: summarizeUsage([...(evidenceUsage || []), ...(guarded.usage || [])])
  };
}
